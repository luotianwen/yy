...
import com.shifeng.util.ReflectHelper;
...
public class SQLServerPagePlugin implements Interceptor {
			...
			if(mappedStatement.getId().matches(pageSqlId)){ //拦截需要分页的SQL
				...
				if(parameterObject==null){
					...
					if("sqlserver".equals(dialect)){
						countSql = "select count(0) from (" + sql.replace("select", " Select  TOP (100) PERCENT  ")+ ")  tmp_count"; 
					}
	...
	private String generatePageSql(String sql,Page page){
		if(page!=null && Tools.notEmpty(dialect)){
			...
			if("mysql".equals(dialect)){
				...
				return buildPageSqlForSQLServerDialect(sql, page);
			}
    ...
    /**
     * 参考hibernate的实现完成SQLServer的分页
     * 
     * @param sql
     * @param page
     * @return String
     */
	private static String buildPageSqlForSQLServerDialect(String sql,Page page) {
        SQLServerDialect dialect = new SQLServerDialect(); 
        String sqlbuild = dialect.getLimitString(sql, page.getCurrentResult(), page.getPageSize());
