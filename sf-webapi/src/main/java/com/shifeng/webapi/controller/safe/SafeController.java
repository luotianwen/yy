...
public class SafeController extends BaseController{
	...
	public ReqResponse<String> checkPhoneExists(String version,String ticket,String phone,String sign){
		...
		//是否能继续获取访问
		if(!this.visitInspect(ticket, "safe/checkPhoneExists", req)){
			return req;
		}
		//验证版本号
		switch (version) {
	...
	public ReqResponse<String> mobileCode(String version,String ticket,String phone,String sign){
		ReqResponse<String> req = new ReqResponse<String>();
		//是否能继续获取访问
		if(!this.visitInspect(ticket, "safe/notifyuser/mobileCode", req)){
			return req;
		}
		//验证版本号
		switch (version) {
						...
						if(m_val == null){//没有发送
							...
							if(count >= Constant.DAY_MAX_SEND_MOBILECODE_COUNT){
								...
								if(isSend){//短信发送成功
									...
									//找回密码手机号十分钟内有效
									RedisTool.expire(findPwd_phone_key, Constant.AUTHCODE_VALID_TIME);
	...
	public ReqResponse<String> checkMobileCode(String version,String ticket,String authcode,String sign){
		ReqResponse<String> req = new ReqResponse<String>();
		//是否能继续获取访问
		if(!this.visitInspect(ticket, "safe/checkMobileCode", req)){
			return req;
		}
		//验证版本号
		switch (version) {
	 ...
	 * @param sign	md5(version+ ticket + newPassword)
	 * @return
	 */
	@RequestMapping(value = "/findPwd")
	@ResponseBody
	public ReqResponse<String> findPwd(String version,String ticket,String newPassword,String sign){
		ReqResponse<String> req = new ReqResponse<String>();
		//是否能继续获取访问
		if(!this.visitInspect(ticket, "safe/findPwd", req)){
			return req;
		}
		//验证版本号
		switch (version) {
			case ApiVersion.V_1_0_0:
				//验证签名
				if(this.verifySign(sign, version+ticket+newPassword,req)){
