						...
						<form id="coupons" method="post" class="form-horizontal ">
							<input type="hidden" id="id" name="id" value="${id}"/>
							   ...
							   <div class="form-group">
									...
									<div class="col-sm-5">
										<input type="text" class="form-control" placeholder="优惠券名称" id="name" name="name" maxlength="50" :value="coupons!=null?coupons.name:''">
							...
							<div class="hr-line-dashed"></div>
							   <div class="form-group">
									<label class="col-sm-2 control-label">类型：</label>
									 <div class="col-sm-5">
		                                        <label>
		                                            <input type="radio"  :checked="coupons!=null&&coupons.type== 1"  value="1" name="type" class="radio i-checks"> <i></i> 优惠券
		                                        </label> &nbsp;
		                                        <label>
							...
							<div class="form-group">
								<div class="col-sm-4 col-sm-offset-2">
									<a class="btn btn-primary" id="submit" @click="submit">写好了，创建</a>
			...
			mounted : function() {
				...
				$.post("coupons/findCouponsById.json?id="+$("#id").val(),function(data) {
					...
					if(_this.coupons!='undefined'&&_this.coupons!=null){
						_this.isShow = _this.coupons.scope;
						_this.selectScope(_this.isShow);
					}
				})
			},
			methods : {
						...
						if(_this.isShow=="2"||_this.isShow=="3"){
							...
							if(_this.shopinfoc!='undefined'&&_this.shopinfoc!=null){
								for(var i=0,len=_this.shopinfoc.length;i<len;i++){
									if(i==0){
										str = "?range="+_this.shopinfoc[i].sku;
									}else{
										str += ","+_this.shopinfoc[i].sku;
									}
								}
							}
				...
				selectScope : function(num){
					...
					if (num == 2 || num == 3) {
						...
						if(_this.coupons!='undefined'&&_this.coupons!=null&&_this.coupons.scope==num){
				...
				selectCategory : function(){
					...
					if(index!=null){
						var _this = this;
						_this.categorysc = _this.categorys[index].nodes;
					}
				},
				...
				queryShop : function(){
					//,$('#shop').serialize()
					spage(1,"shopinfo/findAllShopinfo.json",this,"shopPage");
				},
				checkalls : function(){
					var _this = this;
					if(_this.checks){
				...
				checkboxs : function(){
					...
					for(var i=0,len=_this.checkboxModels.length;i<len;i++){
						var bool = true;
						for(var j=0,lenj=_this.shopinfoc.length;j<len;j++){
							if(_this.shopinfoc[j].name==_this.shopinfos[_this.checkboxModelp[i]].name){
								bool = false;
							}
						}
						if(bool){
							_this.shopinfoc.push(_this.shopinfos[_this.checkboxModelp[i]]);
						}
