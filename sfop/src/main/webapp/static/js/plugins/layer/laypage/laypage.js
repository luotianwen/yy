 ...
 * @param form
 */
function spage(curr,url,_this,id,form){
	if(id==null||id=="")
		id="page";
	$.post(url+"?page="+curr,form,function(data){
		for(var key in data){
		...
		var allpage = _this.page.totalPage;
		laypage({
			cont: id,
			pages: allpage,
			curr: curr,
			...
			jump: function(obj, first){
				if(!first){
					spage(obj.curr,url,_this,id,form);
				}
			}
		});
	})
 ...
 * @param url
 * @param totalPage 总页数
 * @param contentid 替换div内容ID
 * @param pageid 分页div ID
 */
function setPage(curr,url,totalPage,contentid,form,pageid){
	...
	laypage({
		...
		jump: function(obj, first){
			if(!first){
				jpage(obj.curr,url,contentid,form)
			}
		}
	});
}

function jpage(curr,url,contentid,form){
	/*if(form!=null && form!=""){
		form = $("#form").serialize();
	}*/
	$.post(url+"?page="+curr,form,function(data){
