...
function uploader(id,container){
			...
			FilesAdded: function(up, files) {
				plupload.each(files, function(file) {
					makeThumb(file, function (imgsrc) {
						$('#'+container).find("img").css("width","300px").attr({"src":imgsrc,"name":file.name});
						$("#"+id).html("更换图片");
	                    up.start();
	                })
				})
...
var uploaderL = uploader("uploadImgL","uploadL");
uploaderL.init();

var uploaderL = uploader("uploadImgZ","uploadZ");
uploaderL.init();