...
<div class="form-info">
	<form class="form-horizontal" id="returnaddress">
		<input type="hidden" name="id" value="${returnaddress.id }" />
		<div class="form-group">
			<label class="col-sm-2 control-label">联系人：</label>
			<div class="col-sm-10">
				<input type="text" class="form-control" id="shipper" required name="shipper" style="width: 30%;" value="${returnaddress.shipper }">
			</div>
		</div>
		<div class="form-group">
			<label class="col-sm-2 control-label">联系电话：</label>
			<div class="col-sm-10">
				<input type="number" class="form-control" id="phone" required name="phone" style="width: 30%;" value="${returnaddress.phone }">
			</div>
		</div>
		<div class="form-group">
			<label class="col-sm-2 control-label">邮编：</label>
			<div class="col-sm-10">
				<input type="number" class="form-control" id="zipcode" maxlength="6" required name="zipcode" style="width: 30%;" value="${returnaddress.zipcode }">
			</div>
		</div>
		<div class="form-group">
			...
			<div class="col-sm-10">
				<select class="form-control inline" style="width: 15%;" required id="province" name="province">
					...
					<c:forEach items="${province }" var="item">
						<option value="${item.id }" <c:if test="${item.id==returnaddress.province }">selected</c:if>>${item.name }</option>
					</c:forEach>
				</select>
		...
		<div class="form-group">
			<label class="col-sm-2 control-label">退货详细地址：</label>
			<div class="col-sm-10">
				<input type="text" class="form-control" id="address" required name="address" style="width: 50%;" value="${returnaddress.address }">
			</div>
		</div>
		<div class="form-group">
...
<script type="text/javascript">
	var city = "${returnaddress.city}";
	var region = "${returnaddress.region}";
	$(function(){
		$.ajaxSetup({   
		    async : false
		});
		...
		if("${returnaddress.province}"!=""){
			$("#province").trigger("change");
			$("#city").trigger("change");
		}
		
		$("#save").on("click",function(){
			if($("#returnaddress").valid()){
				layer.load(0, {
					shade : 0.3
				});
				...
				$.post("returnaddress/returnAddressEdit.json"+str,$("#returnaddress").serialize(),function(data){
					...
					if (data.RESPONSE_STATE=='200') {
						layer.msg('保存成功', {
							...
							$("#returnaddressEdit").removeClass("in").removeClass("active");
							$("#returnaddressList").addClass("in").addClass("active");
