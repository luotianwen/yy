		...
		FilesAdded: function(up, files) {
			plupload.each(files, function(file) {
				makeThumb(file, function (imgsrc) {
					$("#logosrc").css({"width":"80px","height":"80px"})
                    $("#logosrc").attr("src",imgsrc);
                    $("#upload").css("margin-bottom","10px");
                    html = '<div id="'+file.id+'" style="margin-top: -2px;position: absolute;">' +
			...
			if(data.status == "200"){
				...
				if(json.state=="SUCCESS"){
					if(json.url!=null&&json.url!=''){
						$("#"+file.id).remove();
						$("#upload").css("margin-bottom","");
						$("#logo").val(json.url);
						$("#uploadImg").html("更改LOGO");
					}
				}else{
